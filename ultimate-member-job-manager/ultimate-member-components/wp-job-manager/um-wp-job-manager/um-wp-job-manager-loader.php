...
class UM_WP_Job_Manager_Component {
	...
	public function __construct() {
		...
		add_action('um_profile_content_job_manager_applied', array( $this, 'um_profile_subnav_content_applied_default' ));
		add_action('um_profile_content_job_manager_notes', array( $this, 'um_profile_subnav_content_notes_default' ));
	}

	function jobs_tab( $tabs ) {
		$sub_nav = array(
			'job_dashboard' => __('Job Dashboard','ultimate-member-job-manager'),
			//'jobs' => __('Jobs','ultimate-member-job-manager'),
			//'post_a_job' => __('Post a Job','ultimate-member-job-manager'),
		);
		...
		// Add Post a Job nav item.
		if ( get_current_user_id() == um_profile_id() ) {
			$sub_nav['applied'] = __('Applied','ultimate-member-job-manager');
		}

		// Add Post a Job nav item.
		//hACK FOR NOW - NEED TO CHECK UM_PROFILE
		if ( get_current_user_id() == um_profile_id() && um_user('role') == 'admin') {
			echo "You see this because you are the admin.";
			$sub_nav['notes'] = __('Notes','ultimate-member-job-manager');
		}
	...
	function um_profile_subnav_content_applied_default( $args ) {
		 // include  file or echo do_shortcode
    	 // echo do_shortcode( '[job_alerts]' );
		 get_job_manager_template_part( 'um-applied', 'content', 'um_job_manager', ULTIMATE_MEMBER_WP_JOB_MANAGER_PLUGIN_DIR . '/templates/' );
	}

	function um_profile_subnav_content_notes_default( $args ) {
		 // include  file or echo do_shortcode
    	 // echo do_shortcode( '[job_alerts]' );
		 get_job_manager_template_part( 'um-notes', 'content', 'um_job_manager', ULTIMATE_MEMBER_WP_JOB_MANAGER_PLUGIN_DIR . '/templates/' );
	}
